...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  // Context menu: plain verbs, no icons. Chrome already shows the extension
  // name above them, so the titles do not repeat it.
  chrome.contextMenus.removeAll(() => {
    chrome.contextMenus.create({
      id: "menu_inspect_pii",
      title: "Show what would be hidden here",
      contexts: ["page", "selection", "editable"],
    });
    chrome.contextMenus.create({
      id: "menu_open_hub",
      title: "Open Privacy Agent",
      contexts: ["page"],
    });
    chrome.contextMenus.create({
      id: "menu_open_live_view",
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  } else if (info.menuItemId === "menu_open_live_view") {
    // The side-by-side view was previously reachable only by typing its URL.
...
/* Toolbar badge. Green is "protecting", grey is "paused" — a count is not a
   warning, so finding four things to hide is not painted red. Dark text on a
   light chip is the same inversion the popup uses for its primary button. */
function updateBadge(enabled, piiCount = 0) {
  const paint = (text, background) => {
    chrome.action.setBadgeText({ text });
    chrome.action.setBadgeBackgroundColor({ color: background });
    if (chrome.action.setBadgeTextColor) {
      chrome.action.setBadgeTextColor({ color: "#131313" });
    }
  };
  ...
  if (!enabled) {
    ...
    paint(piiCount > 0 ? `${piiCount}` : "on", "#7dd6a0");
  }
}
