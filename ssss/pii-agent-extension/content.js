...
/* Injected UI shares the extension's tokens, inlined because anything we put
   on someone else's page cannot reach popup.css. Same values, same meanings:
   green is protected, red is a sensitive region, nothing else is tinted. */
const UI = {
  ...
  textMuted: "#6b6b69",
  on: "#7dd6a0",
  onWash: "rgba(125, 214, 160, 0.18)",
  danger: "#e0776b",
  dangerWash: "rgba(224, 119, 107, 0.2)",
  dangerLine: "rgba(224, 119, 107, 0.5)",
  font:
    'ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI Variable Text", "Segoe UI", Roboto, Helvetica, Arial, sans-serif',
};
...
function updateFloatingBadge(piiCount) {
  ...
  if (!badge) {
    ...
    badge.title = "Privacy Agent — open";

    // The font shorthand is set last on purpose: it resets family, weight and
    // line-height together, so a host page's own div rules cannot bleed in.
    Object.assign(badge.style, {
      ...
      font: "500 11.5px/1.4 " + UI.font,
    });

    // A dot, a word, a number. No emoji, no glow, no second colour.
    ...
    count.dataset.paCount = "";
    count.style.color = UI.textMuted;

    badge.append(dot, label, count);

    badge.addEventListener("mouseenter", () => {
      badge.style.borderColor = UI.hairlineStrong;
    });
    badge.addEventListener("mouseleave", () => {
      badge.style.borderColor = UI.hairline;
    });
  ...
  // Only the state changes on a re-scan, so the chip is repainted, not rebuilt.
  const dot = badge.querySelector("[data-pa-dot]");
  const count = badge.querySelector("[data-pa-count]");
  if (dot) dot.style.background = piiCount > 0 ? UI.on : UI.textMuted;
  if (count) {
    count.textContent =
      piiCount > 0 ? `· ${piiCount} hidden` : "· nothing sensitive";
  }
}
...
function animateActionTarget(target, actionType) {
  ...
  Object.assign(indicator.style, {
    ...
    border: "2px solid " + UI.on,
    background: UI.onWash,
    zIndex: "2147483647",
    pointerEvents: "none",
    borderRadius: "4px",
...
function drawHighlightOverlay(matches) {
  ...
  matches.forEach(({ x, y, width, height, reason }) => {
    ...
    Object.assign(box.style, {
      ...
      border: "2px solid " + UI.dangerLine,
      borderRadius: "4px",
      boxSizing: "border-box",
    });

    // Same red-on-dark pill as the "Page" tag in the telemetry table, so a
    // highlight on the page and a row in the audit log read as one thing.
